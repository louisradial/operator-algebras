...
\end{remark}
\begin{proposition}{Sufficient and necessary condition for an element in the adjoint domain}{adjoint_domain}
    Let \(\hilbert\) be a Hilbert space. If \(T : \domain{T} \subset \hilbert \to \hilbert\) is a densely defined operator, then the domain of its adjoint is \(\domain{T^*} = \setc*{v \in \hilbert}{\exists M \in \mathbb{R} : \abs{\inner{T u}{v}} \leq M\norm{u},\forall u \in \domain{T}}\).
\end{proposition}
\begin{proof}
    For \(v \in \hilbert,\) we consider the linear functional
    \begin{align*}
        \lambda_u : \domain{T} &\to \mathbb{C}\\
                          v &\mapsto \inner{u}{Tv}
    \end{align*}
    and define \(D\) as the set of vectors \(u \in \domain{T}\) such that \(\lambda_u\) is bounded. With the Riesz representation, we have
    \begin{align*}
        u \in \domain{T^*} &\iff \exists \eta_u \in \hilbert : \inner{u}{Tv} = \inner{\eta_u}{v}, \forall v \in \domain{T}\\
                           &\iff \exists \eta_u \in \hilbert: \lambda_u(v) = \inner{\eta_u}{v}, \forall v \in \domain{T}\\
                           &\iff \exists \eta_u \in \hilbert: \lambda_u(v) = \riesz^{-1}(\eta_u)v, \forall v \in \domain{T}\\
                           &\iff \exists \eta_u \in \hilbert: \lambda_u = \restrict{\riesz^{-1}(\eta_u)}{\domain{T}}.
    \end{align*}
    Suppose \(u \in \domain{T^*},\) then \(\lambda_u\) is extended by the bounded linear functional \(\riesz^{-1}(\eta_u),\) hence it is bounded, and we have \(u \in D.\) Suppose \(u \in D,\) then \(\riesz^{-1}(\eta_u)\) is the unique bounded extension of \(\ell_u\) to \(\hilbert\), hence \(u \in \domain{T^*}.\) We have thus shown \(D = \domain{T^*},\) as desired.
\end{proof}
\begin{proposition}{Adjoint of an extension}{adjoint_of_extension}
    Let \(\hilbert\) be a Hilbert space. If the densely defined operator \(T : \domain{T} \subset \hilbert \to \hilbert\) is extended by the densely defined operator \(S : \domain{S} \subset \hilbert \to \hilbert,\) then \(T^*\) extends \(S^*.\)
\end{proposition}
\begin{proof}
    Let \(u \in \domain{S^*},\) then there exists \(\eta_u \in \hilbert\) such that \(\inner{u}{Sv} = \inner{\eta_u}{v}\) for all \(v \in \domain{S}.\) In particular, if we take \(v \in \domain{T} \subset \domain{S}\) and replace \(S \to T,\) we conclude \(u \in \domain{T^*},\) that is, \(\domain{T^*} \supset \domain{S^*}.\) Moreover, this also shows \(S^*u = \eta_u = T^*u\) for all \(u \in \domain{S^*},\) which shows \(T^*\) extends \(S^*.\)
\end{proof}
...
\end{definition}
\begin{remark}
    It is important to stress that \(T = T^*\) means \(\domain{T} = \domain{T^*}\) and \(Tu = T^*u\) for all \(u \in \domain{T}\).
\end{remark}
\begin{remark}
...
\end{definition}
\begin{proposition}{Symmetric operator and adjoint extension}{adjoint_extension}
    Let \(\hilbert\) be a Hilbert space. The densely defined operator \(T : \domain{T} \to \hilbert\) is symmetric if and only if \(T^*\) extends \(T\).
\end{proposition}
\begin{proof}
    Suppose \(T\) is symmetric, and let \(u \in \domain{T}.\) Then for all \(v \in \domain{T},\) \(\eta = Tu\) is such that \(\inner{u}{Tv} = \inner{\eta}{v},\) hence \(u \in \domain{T^*},\) that is, \(\domain{T^*} \supset \domain{T}.\) This also shows \(Tu = T^*u\) for all \(u \in \domain{T},\) hence \(T^*\) is an extension of \(T.\)

    Suppose \(T^*\) extends \(T.\) Then for all \(u, v \in \domain{T} \subset \domain{T^*},\) we have
    \begin{equation*}
        \inner{u}{Tv} = \inner{T^*u}{v} = \inner{Tu}{v},
    \end{equation*}
