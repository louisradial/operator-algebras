...
\begin{theorem}{Image of closure and interior under homeomorphism}{closure_interior_homeomorphism}
    Let \topology{X} and \topology{Y} be topological spaces. If \(f : X \to Y\) is a homeomorphism and \(A \subset X\) is a non-empty subset, then \(\cl_Y(f(A)) = f(\cl_XA)\) and \(\inte_Y(f(A)) = f(\inte_XA)\).
\end{theorem}
\begin{proof}
